...
def whatsapp_order(request: Request, db: Session = Depends(get_db)):
    ...
    lines.extend(["", f"Estimated total: Rs. {float(total):,.0f}", "Please confirm availability and delivery."])
    url = f"https://wa.me/{settings.WHATSAPP_NUMBER}?text={quote(chr(10).join(lines))}"
